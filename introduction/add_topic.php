...
else if(isset($_POST['addTopic']) && ($_POST['addTopic'] == '发布'))
{	
	$ttitle=$_POST['ttitle'];
	if((strlen($ttitle) > 0) && isset($_SESSION['cid']))
	{
		...
		$tid = $db->getID();
			...
			$db->insertDB('introduction',$content);
		
		//此处存储在数据库资源的url
		$_SESSION['tjump']=false;
		unset($_SESSION['ttitle']);
		unset($_SESSION['tsrc']);
		unset($_SESSION['tcontent']);
	}
	else
	{
